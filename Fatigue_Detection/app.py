...
model = tf.keras.models.load_model('tools/emotiondetector.h5')

write_data_txt_file = False
...
while True:
    ...
    ctime_fps = time.time()
    fps = 1/(ctime_fps - ptime_fps)
    ptime_fps = ctime_fps
    cv2.putText(image,f"{int(fps)}",(20,35),1,cv2.FONT_HERSHEY_COMPLEX,(0,255,0),2)
